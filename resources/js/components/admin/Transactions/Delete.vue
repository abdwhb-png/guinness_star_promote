...
<template>
    <ConfirmDialog group="deleteTransacs" :style="{ width: '50rem' }"
        :breakpoints="{ '1199px': '75vw', '575px': '90vw' }"></ConfirmDialog>
    <ConfirmsPassword @confirmed="passwordConfirmed = true">
        <Button type="button" icon="pi pi-trash" label="Delete Transactions" size="small" severity="danger"
...
function confirmAction() {
    confirm.require({
        group: `deleteTransacs`,
        message:
            "Do you want to delete the transactions from " +
            dates?.value[0] +
            " to " +
            dates?.value[1] +
            " ?",
...
const submit = () => {
    const url = route("admin.transactions.delete");
        ...
        onSuccess: (page) => {
            toast.add({
                ...
                detail:
                    ...
                    "The transactions have been deleted",
                life: 5000,
            });
        },
        onFinish: async () => {
